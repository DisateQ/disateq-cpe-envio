...
txt_to_json.py
==============
Conversor de TXT (formato APIFAS) a JSON (formato FFEE Platform DisateQ).

ESTADO: Preparado — pendiente de integracion con FFEE Platform.
...
Flujo futuro:
  TXT guardado en enviados/ → convertir a JSON → enviar a FFEE Platform

Uso (cuando este disponible):
  from txt_to_json import txt_a_json, convertir_archivo
  payload = txt_a_json(contenido_txt)
...
def txt_a_json(contenido_txt: str, ruc_emisor: str, razon_social: str) -> dict:
    """
    Convierte el contenido de un TXT APIFAS al formato JSON de FFEE Platform.
    Retorna dict listo para serializar y enviar.
    ...
    payload = {
        ...
        "items": items,
        # Campos para notas de credito/debito (si aplica)
...
def convertir_archivo(ruta_txt: Path, ruc: str, razon_social: str) -> dict:
    """
    Lee un archivo TXT y retorna el payload JSON.
    Util para conversion batch de archivos en enviados/.
    """
    contenido = ruta_txt.read_text(encoding="latin-1")
    return txt_a_json(contenido, ruc, razon_social)


def convertir_carpeta(carpeta: Path, ruc: str, razon_social: str,
                      carpeta_json: Path = None) -> tuple[int, int]:
    """
